...
class BaseApiService {
  constructor(domain, commonHeaders = {}, timeout = 1000, withCredentials = false) {
  ...
  handleErrors(error) {
    ...
    console.log({ message: "Errors in http call", "url": error.request.responseURL, "err": error });
  }

  async post(resource, payload = {}) {
    try {
      const response = await post(resource, payload);
  ...
  errorInterceptor(error) {
    ...
    switch (error.response.status) {
      ...
      default:
        ...
        console.error("maya interceptor default", error.response.status, error.message);
    }
    throw error;
  }
}
