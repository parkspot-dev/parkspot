...
describe('OrganismAccountInformation.vue', () => {
    ...
    it('submits a new UPI account with the full POST payload and emits the payload', async () => {
        ...
        expect(mayaClient.post).toHaveBeenCalledWith(apiPath, expectedPayload);
        expect(mayaClient.patch).not.toHaveBeenCalled();
        expect(wrapper.emitted('saved')).toHaveLength(1);
        expect(wrapper.emitted('saved')[0][0]).toEqual(expectedPayload);
        expect(wrapper.emitted('close')).toBeUndefined();
    });

    it('updates an existing mobile account with the full PATCH payload including AccountID', async () => {
        await remount({
            existingAccount: {
                AccountID: 10,
            },
        ...
        expect(mayaClient.patch).toHaveBeenCalledWith(apiPath, expectedPayload);
    ...
    it('submits a new bank account with the full POST payload and emits the payload', async () => {
        mayaClient.post.mockResolvedValue({ ErrorCode: 0 });
